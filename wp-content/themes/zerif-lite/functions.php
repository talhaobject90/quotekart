...
function zerif_setup()
{
    ...
    register_nav_menus(array(
        ...
        'leftmenu' => __('Left Menu', 'zerif-lite'),
...
function your_wpcf7_mail_sent_function( $contact_form ) {
	...
	if ( $submission ) {
		...
		$submit_time= $submission->get_current_time_stamp();
		
		global $wpdb;
		
		$posted_data_serialized = serialize($posted_data);
		
		$wpdb->insert("wp_quotetable", array(
				"submit_time" => $submit_time,
				"useremail" => $posted_data['your-email'],
				"category" => $title,
				"date" => date('Y-m-d H:i:s'),
				"data" => $posted_data_serialized,
		));
